...
void TcpServer::NewConnection(int sockfd, const InetAddress &peerAddr)
{
    ...
    //设置如何关闭连接的回调 调用TcpConneciton::RemoveConnection
...
{
    LOG(INFO) << " TcpServer::RemoveConnectionInLoop fd: " << conn->Name() << std::endl;
...
TcpServer::~TcpServer()
{
    ...
    {
        // 释放TcpServer保存的TcpConnection 使用一个临时变量维护引用计数
        // 出作用域自动回收资源
        std::shared_ptr<TcpConnection> temp(item.second);
        temp->GetLoop()->QueueInLoop(
            std::bind(&TcpConnection::ConnectDestroyed, temp));
    }
}