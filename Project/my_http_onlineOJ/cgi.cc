...
int main()
{
    ...
    if (method == "GET")
    {
        ...
        else //返回题目页面
        {
            ...
            std::cerr << question_html << std::endl; // DEBUG

            std::cout << question_html;
        }
    }
    else //POST  编译 运行
    {
        ...
        Compiler::CompileAndRun(req_json, &resp_json); //编译 运行

        std::string result_html; //渲染结果页面
        OjView::RenderResult(resp_json["stdout"].asString(), resp_json["reason"].asString(), &result_html);

        std::cerr << result_html << std::endl; //DEBUG
