...
private:
	bool RecvHttpRequestLine()
	{
		if (Util::ReadLine(sock, http_request.request_line))
		{
			...
			line.resize(line.size() - 1); //删除请求行的\n
		}
		else
		{
			stop = true;
...
public:
	...
	bool SendHttpResponse()
	{
		...
		//发送响应空行
		int rd = send(sock, http_response.blank.c_str(), http_response.blank.size(), 0);

		if (rd < 0)
		{
			LOG(ERROR) << "HttpResponse send error!" << std::endl;
			return false;
		}
